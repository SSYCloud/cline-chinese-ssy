...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("shengsuan-cloud.cline-shengsuan")
    ...
    **Note:** To ensure that the `shengsuan-cloud.cline-shengsuan` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:
