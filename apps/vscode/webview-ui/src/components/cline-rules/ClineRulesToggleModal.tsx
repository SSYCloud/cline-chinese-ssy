	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>自定义</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "隐藏自定义" : "显示自定义"}
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								...
								<p>
									技能是可复用的指令集，Cline 可按需激活。当任务与技能描述匹配时，Cline 会使用{" "}
					...
					<div className="flex-1 overflow-y-auto px-3 pb-3" style={{ minHeight: 0 }}>
						{currentView === "rules" ? (
							<>
								...
								{hasRemoteRules && (
									<div className="mb-3">
										<div className="text-sm font-normal mb-2">企业规则</div>
							...
							<>
								...
								{hasRemoteWorkflows && (
									<div className="mb-3">
										<div className="text-sm font-normal mb-2">企业工作流</div>
										<div className="flex flex-col gap-0">
											{remoteWorkflows
							...
							<>
								...
								{isWindows && (
									<div className="flex items-center gap-2 px-3 py-3 mb-4 bg-vscode-inputValidation-warningBackground border-l-[3px] border-vscode-inputValidation-warningBorder">
										<i className="codicon codicon-warning text-sm" />
										<span className="text-base">
											此基础 PR 中尚不支持在 Windows
											上切换钩子。钩子可以创建、编辑和删除，只要钩子文件存在即会执行。接下来：跨平台基于
											JSON 的钩子启用/禁用状态。
										</span>
									</div>
								)}
								...
								{/* Global Hooks */}
								<div className="mb-3">
									<div className="text-sm font-normal mb-2">全局钩子</div>
									<div className="flex flex-col gap-0">
										{globalHooks
											.sort((a, b) => a.name.localeCompare(b.name))
											.map((hook) => (
							...
							<>
								...
								{hasRemoteSkills && (
									<div className="mb-3">
										<div className="text-sm font-normal mb-2">企业技能</div>
										<div className="flex flex-col gap-0">
											{remoteSkills
												.sort((a, b) => a.name.localeCompare(b.name))
												.map((skill) => {
								...
								<div className="mb-3">
									<div className="text-sm font-normal mb-2">全局技能</div>
									<div className="flex flex-col gap-0">
										{globalSkills
								...
								<div className="-mb-2.5">
									<div className="text-sm font-normal mb-2">工作区技能</div>
									<div className="flex flex-col gap-0">
										{localSkills
											.sort((a, b) => a.name.localeCompare(b.name))
											.map((skill) => (
